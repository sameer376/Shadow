...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    if is_owner(uid):
        text = escape_markdown_v2(HELP_OWNER_RAW)
    else:
        text = escape_markdown_v2(HELP_GENERAL_RAW)
    buttons_help = [
...
async def logs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not content.strip():
            await update.message.reply_text("📄 Logs empty.")
...
async def users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        parts = []
        for u in users:
            uid, uname, fname = u.split("|")
            parts.append(f"ID: {uid}\nUsername: @{uname}\nName: {fname}\n---")
        for i in range(0, len(parts), 10):
            await update.message.reply_text("\n".join(parts[i:i+10]))
    except FileNotFoundError:
        await update.message.reply_text("📃 No users file found.")
